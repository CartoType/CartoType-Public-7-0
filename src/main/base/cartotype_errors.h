...
class TResult
    ...
    constexpr TResult(int32_t aCode) noexcept
        {
        *this = aCode;
        }
    ...
    constexpr void operator=(int32_t aCode) noexcept
        {
        iCode = aCode;
        }
...
overlap.
*/
// constexpr TResult KErrorOverlap = 18;
...
*/
// constexpr TResult KErrorCentralPath = 27;
...
constexpr int32_t KStandardErrorCodeCount = 63;

/** Returns a short description of an error, given its code. */
std::string ErrorString(int32_t aErrorCode);

/**
The base of error codes for all errors from the SAX API
to the XML parser and to the XML parser subsystem. It is made from the
The high two bytes of this number are the ASCII codes of the letters 's' and 'a', standing for 'SAX'.
*/
constexpr uint32_t KErrorXmlBase = 0x73610000;
...
*/
constexpr uint32_t KErrorSystem = 0x73790000;

/**
The base of error codes for returning SQLite errors.
The high two bytes of this number are the ASCII characters 'S' and 'Q'.
The SQLite error code itself is placed in the lower two bytes.
*/
constexpr uint32_t KErrorSQLite = 0x83810000;
